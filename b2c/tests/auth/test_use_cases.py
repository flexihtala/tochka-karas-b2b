...
from apps.auth.use_cases.register_buyer import RegisterBuyerUseCase
...
def make_settings() -> B2CSettings:
    return B2CSettings(DATABASE_URL='postgresql+asyncpg://postgres:postgres@localhost/postgres')


def register_request(email: str = 'buyer@example.com') -> RegisterBuyerRequestSchema:
    return RegisterBuyerRequestSchema(
        email=email,
...
@pytest.mark.anyio
async def test_register_buyer_creates_user_refresh_token_and_returns_tokens():
    users = FakeUserRepository()
    refresh_tokens = FakeRefreshTokenRepository()
    jwt_service = FakeJwtService()

    use_case = RegisterBuyerUseCase(
    ...
    assert users.created[0].role == UserRole.BUYER
...
@pytest.mark.anyio
async def test_register_buyer_rejects_duplicate_email():
    users = FakeUserRepository()
    users.add(make_user_read_schema(email='buyer@example.com'))
...
@pytest.mark.anyio
async def test_register_buyer_minimal_payload_works_without_last_name_and_phone():
    users = FakeUserRepository()
    refresh_tokens = FakeRefreshTokenRepository()
    ...
    result = await use_case(
        RegisterBuyerRequestSchema(email='buyer@example.com', password='SecurePass123!', first_name='Ivan')
    )

    assert result.user_id in users.users_by_id
    user = users.users_by_id[result.user_id]
...
async def test_login_returns_tokens_for_valid_credentials():
    users = FakeUserRepository()
    user = make_user_read_schema(email='buyer@example.com', password_hash='hashed:SecurePass123!')
    users.add(user)
    refresh_tokens = FakeRefreshTokenRepository()
    ...
    result = await use_case(LoginRequestSchema(email='buyer@example.com', password='SecurePass123!'))
...
async def test_login_rejects_invalid_password():
    users = FakeUserRepository()
    users.add(make_user_read_schema(email='buyer@example.com', password_hash='hashed:SecurePass123!'))
    ...
    with pytest.raises(InvalidCredentialsError):
        await use_case(LoginRequestSchema(email='buyer@example.com', password='WrongPass123!'))


@pytest.mark.anyio
async def test_login_rejects_inactive_user():
    users = FakeUserRepository()
    users.add(make_user_read_schema(email='buyer@example.com', password_hash='hashed:SecurePass123!', is_active=False))
    ...
    with pytest.raises(UserBlockedError):
        await use_case(LoginRequestSchema(email='buyer@example.com', password='SecurePass123!'))
...
@pytest.mark.anyio
async def test_refresh_rejects_unknown_jti():
    jwt_service = FakeJwtService()
    jwt_service.set_decoded(
        'orphan',
        JwtClaimsSchema(
    ...
    with pytest.raises(TokenRevokedError):
        await use_case(RefreshRequestSchema(refresh_token='orphan'))
...
async def test_logout_blacklists_refresh_token_for_current_user():
    ...
    await use_case(
        ...
        AuthenticatedUserSchema(id=user_id, role=UserRole.BUYER),
    )
...
async def test_logout_rejects_refresh_token_from_another_user():
    ...
    with pytest.raises(InvalidTokenError):
        await use_case(
            LogoutRequestSchema(refresh_token='refresh-token'),
            AuthenticatedUserSchema(id=uuid4(), role=UserRole.BUYER),
        )


@pytest.mark.anyio
async def test_logout_is_idempotent_when_token_already_blacklisted():
    user_id = uuid4()
    jti = uuid4()
    jwt_service = FakeJwtService()
    jwt_service.set_decoded(
    ...
    # No new entry should have been created since it was already there.
    assert blacklist.created == []
