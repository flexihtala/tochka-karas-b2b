...
from shared.auth_lib import IssuedTokenSchema, JwtClaimsSchema, TokenPairSchema, UserRole
from shared.auth_lib.jwt_service import JwtExpiredError, JwtInvalidError
...
class FakeUserRepository:
    ...
    async def update(self, data) -> UserReadSchema | None:
        existing = self.users_by_id.get(data.id)
        if existing is None:
            return None
        ...
        updated = UserReadSchema.model_validate(merged)
        self.add(updated)
        return updated
