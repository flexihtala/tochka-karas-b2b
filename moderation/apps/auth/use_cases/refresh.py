...
class RefreshUseCase:
    ...
    async def __call__(self, data: RefreshRequestSchema) -> RefreshTokensResponseSchema:
        ...
        moderator = await self.moderator_repository.get_or_none(claims.sub)
        if moderator is None:
            raise TokenRevokedError()
        if not moderator.is_active:
            raise UserBlockedError()
        if moderator.password_changed_at and int(moderator.password_changed_at.timestamp()) > claims.iat:
            raise TokenRevokedError()
