...
- test_missing_service_key_returns_401 — POST без X-Service-Key → 401.

Доп. покрытие: 204 на MODERATED/BLOCKED, неверный ключ → 401, невалидное
тело → 400, product_id не найден → 404.
"""
...
def test_missing_service_key_returns_401(stub: StubApplyModerationEventUseCase):
    """POST без X-Service-Key → 401 (DoD)."""
    client = TestClient(_make_app(stub))
