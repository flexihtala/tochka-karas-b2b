...
class SessionManager:
    ...
    Каждый сервис создаёт один экземпляр в DI-контейнере с собственным Settings
    (т.е. со своим database_url). Не разделяется между сервисами.
    """

    def __init__(self, settings: DBSettingsProtocol):
    ...
    async def get_session(self) -> AsyncIterator[AsyncSession]:
        """Открывает транзакцию. commit/rollback автоматический."""
