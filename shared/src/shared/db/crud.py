"""Generic CRUD-repository, основанный на trinity (Model, CreateSchema, ReadSchema, UpdateSchema)."""
...
class DBCrudRepository(Generic[ModelType, CreateSchemaType, ReadSchemaType, UpdateSchemaType]):
    ...
    Конкретный репозиторий объявляет дженерик-параметры:
    ...
    async def get_or_none(self, id_: Any) -> ReadSchemaType | None:
    ...
    async def update(self, data: UpdateSchemaType) -> ReadSchemaType | None:
        ...
        query = update(self.model_type).where(self._id_field == data.id).values(**values).returning(self.model_type)  # type: ignore[attr-defined]
